"""
Render "RB" as an EXTRUDED 3D wordmark rasterized to ASCII, and emit it as an
...
def pick_font():
    explicit = os.environ.get("WORDMARK_FONT")
    if explicit and os.path.exists(explicit):
        return explicit, int(os.environ.get("WORDMARK_FONT_INDEX", 0))
    ...
    for path, index in candidates:
        if os.path.exists(path):
            return path, index
    raise FileNotFoundError("no usable font found; set WORDMARK_FONT to a bold font file")
...
TEXT = os.environ.get("WORDMARK_TEXT", "RUPESHRB")
...
def emit(frames, mode, out, dur, reveal):
    ...
    p.append(f'<text x="{canvas_w/2:.0f}" y="{TITLEBAR_H/2 + 4:.0f}" fill="{TITLE_TEXT}" '
             f'font-size="11.5" text-anchor="middle">RB@github: ~$ ./wordmark.sh --3d</text>')
